...
router.route("/create").post(validate(createCategorySchema), authenticate, verifyAdmin, categoryController.createCategory)
router.route("/get-all-category").get(categoryController.getAllCategory)

router.route("/:catId").patch(validate(updateCategorySchema),authenticate, verifyAdmin, categoryController.updateCategory)
